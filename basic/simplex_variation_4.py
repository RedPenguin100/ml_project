...
h_fully_connected_drop = tf.nn.dropout(h_fully_connected1, keep_prob, seed=deterministic_seed)

# Second fully connected layer
weights_fully_connected2 = get_weights([1024, num_classes - 1])
biases_fully_connected2 = get_bias([num_classes - 1])

h_fully_connected2 = tf.matmul(h_fully_connected_drop, weights_fully_connected2) + biases_fully_connected2
...
with tf.Session(config=config) as sess:
    ...
    if sys.argv[1] == "train":
        ...
        for i in range(number_of_iterations):
            ...
            if i % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={x: batch_images, y_true_simplex: batch_simplex, keep_prob: 1})
                print("step %d, training accuracy: %.4f" % (i, train_accuracy))
            if i % 1000 == 0:
                saver.save(sess=sess, save_path=SAVED_MODEL_PATH)
            optimizer.run(feed_dict={x: batch_images, y_true_simplex: batch_simplex, keep_prob: 0.5})
        saver.save(sess=sess, save_path=SAVED_MODEL_PATH)
        ...
        batch_size = 200
        layer_file_names = ["simplex_4variation_1.txt", "simplex_4variation_2.txt", "simplex_4variation_3.txt",
                            "simplex_4variation_4.txt"]
