...
with tf.Session(config=config) as sess:
    ...
    if sys.argv[1] == "train":
        ...
        flatten_h_pool3 = tf.reshape(h_conv3, shape=[-1, 7 * 7 * 64])

        h_fully_connected1 = tf.reshape(h_fully_connected1, shape=[-1, 1024])
        h_fully_connected2 = tf.reshape(h_fully_connected2, shape=[-1, 9])
        batch_size = 200
        layer_file_names = ["simplex_1variation_1.txt", "simplex_1variation_2.txt", "simplex_1variation_3.txt",
                            "simplex_1variation_4.txt", "simplex_1variation_5.txt"]
