...
weights3 = get_weights([5, 5, 64, 128])
biases3 = get_bias([128])
h_conv3 = tf.nn.relu(tf.nn.conv2d(h_pool2, weights3, strides=[1, 1, 1, 1], padding='SAME') + biases3)

# First fully connected layer
weights_fully_connected1 = get_weights([7 * 7 * 128, 1024])
biases_fully_connected1 = get_bias([1024])

h_pool3_flat = tf.reshape(h_conv3, [-1, 7 * 7 * 128])
h_fully_connected1 = tf.nn.relu(tf.matmul(h_pool3_flat, weights_fully_connected1) + biases_fully_connected1)
...
with tf.Session(config=config) as sess:
    ...
    if sys.argv[1] == "train":
        ...
        flatten_h_pool3 = tf.reshape(h_conv3, shape=[-1, 7 * 7 * 128])
        h_fully_connected1 = tf.reshape(h_fully_connected1, shape=[-1, 1024])
        h_fully_connected2 = tf.reshape(h_fully_connected2, shape=[-1, 9])
        batch_size = 200
        layer_file_names = ["simplex_2variation_1.txt", "simplex_2variation_2.txt", "simplex_2variation_3.txt",
                            "simplex_2variation_4.txt", "simplex_2variation_5.txt"]
        ...
        for i in range(len(data.train.images) // batch_size):
            ...
            np.savetxt(file_handles[2], flatten_h_pool3.eval(feed_dict=dict_to_feed), fmt="%f")
            np.savetxt(file_handles[3], h_fully_connected1.eval(feed_dict=dict_to_feed), fmt="%f")
            np.savetxt(file_handles[4], h_fully_connected2.eval(feed_dict=dict_to_feed), fmt="%f")
            if i % 10 == 0:
