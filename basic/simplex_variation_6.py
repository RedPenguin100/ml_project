...
weights_fully_connected1 = get_weights([7 * 7 * 32, 1024])
...
with tf.Session(config=config) as sess:
    ...
    if sys.argv[1] == "train":
        ...
        flatten_h_pool1 = tf.reshape(h_pool1, shape=[-1, 14 * 14 * 16])
        flatten_h_pool2 = tf.reshape(h_pool2, shape=[-1, 7 * 7 * 32])
        h_fully_connected1 = tf.reshape(h_fully_connected1, shape=[-1, 1024])
        h_fully_connected2 = tf.reshape(h_fully_connected2, shape=[-1, 9])
        batch_size = 200
        layer_file_names = ["simplex_6variation_1.txt", "simplex_6variation_2.txt", "simplex_6variation_3.txt",
                            "simplex_6variation_4.txt"]
