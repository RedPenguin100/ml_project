...
weights2 = get_weights([5, 5, 32, 128])
biases2 = get_bias([128])
...
with tf.Session(config=config) as sess:
    ...
    if sys.argv[1] == "train":
        ...
        flatten_h_pool2 = tf.reshape(h_pool2, shape=[-1, 7 * 7 * 128])
        h_fully_connected1 = tf.reshape(h_fully_connected1, shape=[-1, 1024])
        h_fully_connected2 = tf.reshape(h_fully_connected2, shape=[-1, 9])
        batch_size = 200
        layer_file_names = ["simplex_5variation_1.txt", "simplex_5variation_2.txt", "simplex_5variation_3.txt",
                            "simplex_5variation_4.txt"]
